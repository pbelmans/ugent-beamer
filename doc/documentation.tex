...
In case you're using Unix and/or \TeX Live you need to have a local directory (this will probably be \verb|~/texmf/|) and you need to place all the files from the \verb|theme/| folder in the directory \verb|~/texmf/tex/latex/beamer/themes/UGent/|, finishing it by running \verb|texhash|.
...
\begin{lstlisting}[float,caption={A minimalistic test file for the UGent Beamer theme.},captionpos=b,label=listing:minimal-example,abovecaptionskip=\bigskipamount]
...
\newlength\colorwidth
\setlength\colorwidth{2.5cm}
\setlength\figurewidth{6mm}
\begin{table}
  \centering
  \begin{tabular}{cp{\facultieswidth}p{\colorwidth}c}
    ...
    & \parbox\colorwidth{Pantone 3262c \\ \extractcolorspecs{ugent-pp}\modelspec\colorspec \convertcolorspec\modelspec\colorspec{RGB}\colorspec RGB\colorspec}
    ...
    & \parbox\colorwidth{Pantone purple \\ \extractcolorspecs{ugent-la}\modelspec\colorspec \convertcolorspec\modelspec\colorspec{RGB}\colorspec RGB\colorspec}
    ...
    & \parbox\colorwidth{Pantone 375c \\ \extractcolorspecs{ugent-lw}\modelspec\colorspec \convertcolorspec\modelspec\colorspec{RGB}\colorspec RGB\colorspec}
...
\begin{figure}
  ...
  \caption{Example usage of the official colors within a set of graphs, using also an exception color, which happens to be \lstinline|ugent-re|.}
  \label{figure:coloredgraphs}
\end{figure}

If you need to emphasize a particular aspect in your slides (graphs, tables), you can (within \texttt{beamer}) use the \lstinline!\alert{}! macro (e.g., \lstinline!\alert{This is alerted text.}!). For the situation where something needs to stick out in a pie chart, for example, where the ordinary colors (UGent blue and yellow) have been used up already, you can choose to use the color of your faculty (if specified) which is available in \lstinline|ugent-faculty| or choose a color from Table~\ref{table:faculty-values} as these have been designed to match the standard colors of UGent. It is to be used scarcely and strictly for highlighting purposes (see, for example, Figure \ref{fig:coloredgraphs}).
